...
CREATE VIEW users_groups_view  AS select users.id as id, COALESCE(users.user_data,groups.group_data) as ho_jsonb  from users inner JOIN groups ON users.user_data->>'groupId' = groups.group_data->>'id';
