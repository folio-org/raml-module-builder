...
public class Raml2Java extends AbstractGeneratorExtension {
  ...
  private void handleOverrides() throws IOException {
    String overrides = null;
    if(overrideFileExists){
      overrides = IOUtils.toString(getClass().getClassLoader().getResourceAsStream(
    ...
    if(overrideEntry != null){
      ...
      for (i[0] = 0; i[0] < params.length; i[0]++) {
        ...
        entries.forEach( entry -> {
          ...
          System.out.println(" ---------------- url: " + action.getResource().getUri());
          ...
          if(!action.getResource().getUri().equalsIgnoreCase(job.getString("verb"))){
            ...
            System.out.println(" paramName  ---------------> "+params[i[0]].name());
            ...
            if(paramName.equalsIgnoreCase(params[i[0]].name())){
              ...
              params[i[0]].annotations().forEach( use -> {
                System.out.println(" defined ---------------> "+ use.getAnnotationClass().name());
                ...
                if(annotationType != null && annotationType.endsWith(use.getAnnotationClass().name())){
                  found[0] = true;
                  System.out.println(" ---------------> found existing annotation " + type);
                  ann[0] = use;
                }
                System.out.println(" ---------------> "+use.getAnnotationClass().name());
              });
              if(!found[0]){
                //annotation not found for this paramter, add it
                System.out.println(" ---------------> new annotation " + type);
                JClass annClazz = new JCodeModel().ref(Enum2Annotation.getAnnotation(type));
                ann[0] = params[i[0]].annotate(annClazz);
              }
              if(ann[0].getAnnotationClass().name().equals("Size")){
                ...
                for (int j = 0; j < multipleAnnos.length; j++) {
                  if(j == 0){
                    ann[0].param("min", Integer.valueOf(multipleAnnos[j].trim()));
                  }
                  else{
                    ann[0].param("max", Integer.valueOf(multipleAnnos[j].trim()));
                  }
                }
              }
                ...
                if(value instanceof String){
                  if(type.equalsIgnoreCase("PATTERN")){
                    ann[0].param("regexp", (String)value);
                  }
                  else{
                    ann[0].param("value", (String)value);
                  }
                }
                else if(value instanceof Boolean){
                  ann[0].param("value", (Boolean)value);
                }
                else if(value instanceof Integer){
                  ann[0].param("value", (Integer)value);
                }
              }
            }
          }
