...
public class ResourceMethodExtensionPlugin implements ResourceMethodExtension<GMethod> {
  ...
  private void addRoutingContext(Builder methodSpec, GMethod method) throws Exception {
    ...
    System.out.println("checking whether routing context is needed");
    if(endpoints2addRoutingContext != null){
      ...
      for (int i = 0; i < rcFuncs.length; i++) {
        try {
          ...
          System.out.println("checking whether routing context is needed for " + rcFuncs[i] + " : " +method.resource().resourcePath());
...
*/          }
  ...
  private void generateJavaDocs(GMethod method, MethodSpec.Builder methodSpec){
    ...
    for(int i=0; i<methodParams.size(); i++){
      String paramName = methodParams.get(i).name();
      methodSpec.addJavadoc("@param "+paramName.replace("\"", ""));
      String defaultVal = methodParams.get(i).defaultValue();
      MarkdownString desc = ((TypeDeclaration) methodParams.get(i).implementation()).description();
      if(desc != null){
          ...
          if(verb.equalsIgnoreCase(job.getString("verb"))){
            ...
            if(paramName.equalsIgnoreCase(paramSpec.get(i[0]).name)){
              ...
              for(int j=0; j<originalAnnotations.size(); j++ ){
                System.out.println(Enum2Annotation.getAnnotation(type));
                System.out.println(originalAnnotations.get(j).type.toString());
