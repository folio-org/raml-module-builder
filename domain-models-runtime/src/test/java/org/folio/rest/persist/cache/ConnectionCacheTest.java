...
public class ConnectionCacheTest {
  ...
  public void getConnectionAvailableOfCurrentTenant() {
    ...
    var conn1 = new CachedPgConnection("tenant1", "schema1", new PgConnectionMock(), manager, vertx, 1);
    var conn2 = new CachedPgConnection("tenant2", "schema2", new PgConnectionMock(), manager, vertx, 1);
    var conn3 = new CachedPgConnection("tenant3", "schema3", new PgConnectionMock(), manager, vertx, 1);
    cache.tryAdd(conn1);
    cache.tryAdd(conn2);
    cache.tryAdd(conn3);
    conn2.close(); // Set it to available.

    var connOptional = cache.getAvailableConnection("tenant2", "schema2");
  ...
  public void getConnectionOldestAvailableOfAnotherTenant() {
    ...
    var vertx = Vertx.vertx();
    var conn1 = new CachedPgConnection("tenant1", "schema1", new PgConnectionMock(), manager, vertx, 1);
    var conn2 = new CachedPgConnection("tenant3", "schema3", new PgConnectionMock(), manager, vertx, 1);
    var conn3 = new CachedPgConnection("tenant4", "schema4", new PgConnectionMock(), manager, vertx, 1);
    cache.tryAdd(conn1);
    cache.tryAdd(conn2);
    cache.tryAdd(conn3);
  ...
  public void getConnectionWhenNoneAreAvailable() {
    ...
    var connOptional1 = cache.getAvailableConnection("tenant1", "schema1");
    assertFalse(connOptional1.isPresent());
    var connOptional2 = cache.getAvailableConnection("tenant2", "schema2");
    assertFalse(connOptional2.isPresent());
    var connOptional3 = cache.getAvailableConnection("tenant3", "schema3");
    assertFalse(connOptional3.isPresent());
    var connOptional4 = cache.getAvailableConnection("tenant4", "schema4");
  ...
  public void tryAddAndRemoveOldestAvailableTest() {
    ...
    var vertx = Vertx.vertx();
    var conn1 = new CachedPgConnection("tenant1", "schema1", new PgConnectionMock(), manager, vertx, 1);
    var conn2 = new CachedPgConnection("tenant1", "schema1", new PgConnectionMock(), manager, vertx, 1);
    var conn3 = new CachedPgConnection("tenant2", "schema2", new PgConnectionMock(), manager, vertx, 1);
    conn2.close(); // Set it to available.
    cache.tryAdd(conn1);
    cache.tryAdd(conn2);
    cache.tryAdd(conn3);
