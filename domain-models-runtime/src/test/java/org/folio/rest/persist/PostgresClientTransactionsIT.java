  ...
  private void updateTransaction(TestContext context) {
    ...
    c1.startTx(handler -> {
      if(handler.succeeded()){
        PgTransaction tx = handler.result();
  ...
  private void deleteTransaction(TestContext context) {
    ...
    c1.startTx( handler -> {
      if(handler.succeeded()){
        ...
        c1.delete(tx.connection(), table, new Criterion(c) , reply -> {
            if(reply.succeeded()){
              ...
              c1.get(tx.connection(), "z", SimplePojo.class , new Criterion(c) ,
                  true, true, reply2 -> {
                if (! reply2.succeeded()) {
                  context.fail(reply2.cause());
                  async.complete();
