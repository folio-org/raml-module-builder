...
public class SchemaMakerTest {
  ...
  public void canCreateIndexPath() throws IOException, TemplateException {
    ...
    assertThat(result,containsString("fail"));
    result = schemaMaker.getSchema().getTables().get(0).getIndex().get(0).createCompoundIndex("tablea");
    assertThat(result,containsString("fail"));
  ...
  public void canCreateIndexPath2() throws IOException, TemplateException {
    ...
    String json = ResourceUtil.asString("templates/db_scripts/compoundIndex.json");
    schemaMaker.setSchema(ObjectMapperTool.getMapper().readValue(json, Schema.class));
    String result = schemaMaker.getSchema().getTables().get(0).getIndex().get(0).createCompoundIndex("tablea");
    assertThat(result,containsString("fail"));
    result = schemaMaker.getSchema().getTables().get(0).getIndex().get(1).createCompoundIndex("tablea");
    assertThat(result,containsString("fail"));
    result = schemaMaker.getSchema().getTables().get(0).getIndex().get(2).createCompoundIndex("tablea");
    assertThat(result,containsString("fail"));
  }
  @Test
