...
public class SchemaMakerTest {
  ...
  public void canCreateCompoundIndex() throws IOException, TemplateException {
    ...
    assertThat(result,containsString("( my_tsvector(concat_space_sql(concat_array_object_values(tablea.jsonb->'field1','firstName') , concat_array_object_values(tablea.jsonb->'field2','lastName'))) )"));
    assertThat(result,containsString("( my_tsvector(concat_space_sql(concat_array_object_values(tablea.jsonb->'field1'->'info','firstName') , concat_array_object_values(tablea.jsonb->'field2'->'info','lastName'))) )"));
  }
  ...
  public void ginIndex() throws IOException, TemplateException {
    ...
    assertThat(ddl, containsString("GIN ' || $rmb$(my_tsvector(f_unaccent(jsonb->>'title')))"));
    assertThat(ddl, containsString("GIN ' || $rmb$(my_tsvector((jsonb->>'author')))"));
  }
