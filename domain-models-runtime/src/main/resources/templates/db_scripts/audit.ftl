    ...
    BEGIN
        IF (TG_OP = 'DELETE') THEN
          <#if table.auditingSnippet.delete??>
            ${table.auditingSnippet.delete}
          </#if>
            INSERT INTO ${myuniversity}_${mymodule}.audit_${table.tableName} SELECT gen_random_uuid(), OLD.id, 'D', OLD.jsonb, current_timestamp;
            RETURN OLD;
        ELSIF (TG_OP = 'UPDATE') THEN
          <#if table.auditingSnippet.update??>
            ${table.auditingSnippet.update}
          </#if>
            INSERT INTO ${myuniversity}_${mymodule}.audit_${table.tableName} SELECT gen_random_uuid(), NEW.id, 'U', NEW.jsonb, current_timestamp;
            RETURN NEW;
        ELSIF (TG_OP = 'INSERT') THEN
          <#if table.auditingSnippet.insert??>
            ${table.auditingSnippet.insert}
          </#if>
            INSERT INTO ${myuniversity}_${mymodule}.audit_${table.tableName} SELECT gen_random_uuid(), NEW.id, 'I', NEW.jsonb, current_timestamp;
            RETURN NEW;
        END IF;
        RETURN NULL;
    END;
