...
public class TenantLoading {
    ...
    try {
      ...
      HttpClientRequest reqPut = httpClient.requestAbs(method1, putUri.toString(), resPut -> {
          ...
          if (loadingEntry.strategy != Strategy.RAW_PUT
            ...
            HttpClientRequest reqPost = httpClient.postAbs(endPointUrl, resPost -> {
              if (resPost.statusCode() == 201) {
                ...
                log.error(method1.name() + " " + endPointUrl + RETURNED_STATUS + resPost.statusCode());
                f.handle(Future.failedFuture(method1.name() + " " + endPointUrl
                  + RETURNED_STATUS + resPost.statusCode()));
              }
            });
            reqPost.exceptionHandler(ex -> {
              log.error(method1.name() + " " + endPointUrl + ": " + ex.getMessage());
              if (!f.isComplete()) {
            ...
            log.error(method1.name() + " " + putUri.toString() + RETURNED_STATUS + resPut.statusCode());
            f.handle(Future.failedFuture(method1.name() + " " + putUri.toString()
        ...
        resPut.exceptionHandler(ex -> {
          if (!f.isComplete()) {
            log.error(method1.name() + " " + putUri.toString() + ": " + ex.getMessage());
            if (!f.isComplete()) {
              f.handle(Future.failedFuture(method1.name() + " " + putUri.toString()
                + ": " + ex.getMessage()));
            }
  ...
  private static void loadData(String okapiUrl, Map<String, String> headers,
    ...
    try {
      ...
      boolean useCompose = true;
      Future<Void> future = Future.succeededFuture();
      if (useCompose) {
        for (URL url : urls) {
          future = future.compose(x -> {
            Future n = Future.future();
            loadURL(headers, url, httpClient, loadingEntry, endPointUrl, n);
            return n;
          });
        }
        ...
        log.info("loadData wait with compositeFuture");
