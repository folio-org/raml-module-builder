...
possible and allow a developer to focus on imlpementing business functions, in
...
The framework consists of a number of tools.
 1. domain-models-api-interfaces - project exposes tools that receives as input
...
used to run a module. It is vertx based. Once a developer implements the
...
only need to implement APIs and all the wiring, validation,
...
- When implementing the interfaces add the, you must add the @Validate
...
- The maven exec plugin that will generate the pojos and interfaces based on
the raml files
- The maven resource plugin that will copy the raml files into a directory
...
the `init(Vertx vertx, Context context, Handler<AsyncResult<Boolean>>
resultHandler)`. Currently the implementation should sit in the
...
It is possible to add custom code that will run right before the verticle is
...
default - to change that ass `embed_postgres=true`to the command line (`java
-jar circulation-fat.jar embed_postgres=true`).  Connection parameters to a
...
**Important Note** that the PostgreSQL client implemented currently assumes
...
`.drl.` files. A directory can also be passed via the cmd line. Those files are
...
sends back as error responses to in correct API calls. These messages are
...
in one tab and the data objects (in json format) in a seprate tab.
