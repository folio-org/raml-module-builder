...
11. `withAuditing` - create an auditing table with triggers populating the audit table whenever an insert, update, or delete occurs on the table
    ...
    * The audit table jsonb contains the original jsonb with four special fields added: `origId` contains the original id,
      `id` contains a unique id, `operation` contains `I`, `U`, `D` for insert, update, delete, and `createdDate` contains
